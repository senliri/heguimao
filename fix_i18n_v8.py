...
for i, raw_line in enumerate(raw_lines):
    ...
    # Find opening quote (skip leading space)
    q_start = rest.find(b'"')
    if q_start < 0:
        new_lines.append(raw_line)
        continue
    
    # Find closing quote: last " before },
    # Scan from end to find " followed by optional },
    after_open = rest[q_start + 1:]
    
    # Find the last " that's followed by }, or }
    last_q = -1
    ...
    if last_q >= 0:
        ...
        after_close = b'"'  # add closing quote
    
    # Fix value_content: replace raw UTF-8 bytes with \uXXXX
        ...
        if b > 127:
            ...
            try:
                ...
                if len(char) == 1:
                    ...
                    # Surrogate pair or multi-char — skip
                    pass
