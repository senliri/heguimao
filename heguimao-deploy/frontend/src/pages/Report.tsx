...
export function Report() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      // Title
      doc.setFontSize(20);
      doc.text(t("report.pdf_title"), 14, 20);

      doc.setFontSize(10);
      doc.setTextColor(100);
      doc.text(
        `${market?.label || t("common.us")} · ${category?.label || t("common.general")}${sub ? ` · ${sub.label}` : ""}`,
        14,
        30
      );
      ...
      doc.setFontSize(14);
      doc.setTextColor(0);
      doc.text(t("report.ai_smart_title"), 14, 50);
      doc.setFontSize(9);
      doc.setTextColor(60);
      if (summary) {
        doc.text(summary, 14, 58, { maxWidth: 180 });
      } else {
        doc.text(`${t("report.recommended_actions")}: ${recommendCount}`, 14, 58);
      }
      doc.text(`${t("report.high_risk_count")}: ${highCount}`, 14, 65);

      // Table
      const tableData = displayRecommendations.map((item) => [
        item.name,
        item.required ? t("report.mandatory") : t("report.recommended"),
        item.severity === "high" ? t("report.severity_high") : item.severity === "medium" ? t("report.severity_medium") : t("report.severity_low"),
        item.reason,
        item.action,
      ]);
      ...
      autoTable(doc, {
        startY: 72,
        head: [[t("report.table_cert"), t("report.table_type"), t("report.table_level"), t("report.table_reason"), t("report.table_action")]],
        body: tableData,
        theme: "grid",
      ...
      console.error(t("report.pdf_export_fail"), err);
      alert(t("report.pdf_export_fail"));
    } finally {
  ...
  const handleShare = async () => {
    const shareText = `${t("report.share_title")}\n\n${t("common.product")}: ${aiProduct || (category?.label ? t(`site.category_${category.id}`) : t("common.general"))}\n${t("common.market")}: ${market?.label ? t(`market.label.${market.id}`) : t("common.us")}\n${t("report.total_req")}: ${displayRecommendations.length}\n${t("report.high_pri")}: ${highCount}\n\n${t("report.generated_by")}`;

    if (navigator.share) {
      try {
        await navigator.share({
          title: `${t("report.pdf_title")}: ${(category?.label ? t(`site.category_${category.id}`) : t("common.general"))} - ${(market?.label ? t(`market.label.${market.id}`) : t("common.us"))}`,
          text: shareText,
    ...
    <div>
      <section className="mx-auto mt-6 max-w-7xl px-4 sm:px-6">
        <div className="flex items-center gap-2 text-sm text-slate-400">
          ...
          {catId ? (
            <>
              <Link to={`/category?cat=${catId}`} className="hover:text-white">{category?.label || t("breadcrumb.category")}</Link>
              <span>/</span>
              {subId && (
                <Link to={`/market?cat=${catId}&sub=${subId}`} className="hover:text-white">{market?.label || t("breadcrumb.market")}</Link>
              )}
              <span className="text-slate-200">{t("nav.report")}</span>
            </>
          ) : (
            <span className="text-slate-200">{t("nav.report")}</span>
          )}
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">{t("report.pdf_title")}</h1>
        ...
        <section className="mx-auto mt-6 max-w-7xl px-4 sm:px-6">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-semibold">{t("report.ai_smart_title")}</h2>
      ...
      {activeTab === "compliance" && (
        <section className="mx-auto mt-6 max-w-7xl px-4 sm:px-6">
          <h2 className="text-lg font-semibold">{t("report.checklist")}</h2>
                ...
                <div className="space-y-6">
                  <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
                    ...
                    <div className="space-y-2 text-sm text-slate-300">
                      {marketId === 'eu' && (
                        <div className="space-y-2">
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.eori_num")}:</span> {t("report.eori_desc")}</div></div>
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.vat")}:</span> {t("report.vat_desc")} (DE: Umsatzsteuer-ID, FR: N&#x00b0; TVA).</div></div>
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.epr")}:</span> {t("report.epr_desc")}<div className="mt-1 ml-4 text-xs text-slate-400 space-y-0.5"><div>&#x1F4E6; Packaging: LUCID (DE), &#x00e9;co-EMballages (FR)</div><div>&#x1F50C; WEEE: Register per country for electrical</div><div>&#x1F9D1; Battery: LUCID/BaterReg (DE), InfoBat (FR)</div></div></div></div>
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.eu_rep")}:</span> {t("report.eu_rep_desc")}</div></div>
                        </div>
                      )}
                      {marketId === 'uk' && (
                        <div className="space-y-2">
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.uk_vat")}:</span> {t("report.uk_vat_desc")} &#x00a3;90,000 or via marketplaces.</div></div>
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.uk_rp")}:</span> {t("report.uk_rp_desc")}</div></div>
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.uk_epr")}:</span> {t("report.uk_epr_desc")}</div></div>
                        </div>
                      )}
                      {marketId === 'us' && (
                        <div className="space-y-2">
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.us_reg")}:</span> {t("report.us_reg_desc")}</div></div>
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.us_addr")}:</span> {t("report.us_addr_desc")}</div></div>
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.sales_tax")}:</span> {t("report.sales_tax_desc")}</div></div>
                        </div>
                      )}
                      {marketId === 'jp' && (
                        <div className="space-y-2">
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.jp_tax")}:</span> {t("report.jp_tax_desc")}</div></div>
                          <div className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.consumers")}:</span> {t("report.consumers_desc")}</div></div>
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4">
                    ...
                    <div className="space-y-2 text-sm text-slate-300">
                      <div className="flex items-start gap-2"><span className="text-red-400 mt-0.5">&#x26A0;</span><div><span className="font-medium text-white">{t("report.shell")}:</span> {t("report.shell_desc")}</div></div>
                      <div className="flex items-start gap-2"><span className="text-red-400 mt-0.5">&#x26A0;</span><div><span className="font-medium text-white">{t("report.name_match")}:</span> {t("report.name_desc")}</div></div>
                      <div className="flex items-start gap-2"><span className="text-red-400 mt-0.5">&#x26A0;</span><div><span className="font-medium text-white">{t("report.doc_verify")}:</span> {t("report.doc_desc")}</div></div>
                    </div>
                  </div>
                  <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-4">
                    <h3 className="text-sm font-semibold text-green-300 mb-3">{t("report.lab_section")}</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-slate-300">
                      {[{ name: "SGS", site: "sgs.com", desc: t("lab.sgs_desc"), price: t("price.med_high") }, { name: "&#x00DC;V Rheinland", site: "tuv.com", desc: t("lab.tuv_rheinland_desc"), price: t("price.med_high") }, { name: "Intertek", site: "intertek.com", desc: t("lab.intertek_desc"), price: t("price.medium") }, { name: "CTI", site: "cti-group.com", desc: t("lab.cti_desc"), price: t("price.low_med") }, { name: "Bureau Veritas", site: "bureauveritas.com", desc: t("lab.bureau_veritas_desc"), price: t("price.med_high") }, { name: "Eurofins", site: "eurofins.com", desc: t("lab.eurofins_desc"), price: t("price.high") }].map((lab, idx) => (
                  ...
                  <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4">
                    ...
                    <div className="space-y-2 text-sm text-slate-300">
                      <div className="flex items-start gap-2"><span className="text-blue-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.fcc_id")}</span> {t("report.verify_at")} <a href="https://www.fcc.gov/oet/ea" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">fcc.gov/oet/ea</a></div></div>
                      <div className="flex items-start gap-2"><span className="text-blue-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.ce_doc")}</span> {t("report.ce_v")}</div></div>
                      <div className="flex items-start gap-2"><span className="text-blue-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.cpc")}</span> {t("report.cpc_v")} <a href="https://www.cpsc.gov/CPCTestingLabs" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">cpsc.gov</a></div></div>
                      <div className="flex items-start gap-2"><span className="text-blue-400 mt-0.5">&#x2022;</span><div><span className="font-medium text-white">{t("report.pse")}</span> {t("report.verify_at")} <a href="https://www.nite.go.jp/en/" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">nite.go.jp/en/</a></div></div>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div key={sev}>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className={`text-sm font-semibold ${titleColor}`}>
                        {sev === "high" ? `🔴 High Priority — ${t("report.immediate_action")}` : 
                         sev === "medium" ? `🟡 Medium Priority — ${t("report.handle_soon")}` : 
                         `🟢 Low Priority — ${t("report.handle_order")}`}
                      </span>
                    </div>
                    <div className="space-y-3">
                      {items.map((item, i) => (
                            ...
                            <div className="flex-1">
                              ...
                              <div className="mt-3 flex flex-wrap gap-2">
                                ...
                                <span className="inline-flex items-center gap-1 rounded-md bg-slate-600/20 px-2.5 py-1 text-xs text-slate-400">
                                  <Clock className="h-3 w-3" />{t("report.estimated_time")}: {item.estimatedTime}
                                </span>
      ...
      <section className="mx-auto mt-10 max-w-7xl px-4 pb-16 sm:px-6">
        <div className="rounded-2xl border border-blue-500/20 bg-blue-500/5 p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-blue-300">{t("report.disclaimer_label")}</h3>
