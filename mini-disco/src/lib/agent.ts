// AI Agent core logic — calls Agnes API via Cloudflare Worker in production

import { PROFILE_EXTRACTION_PROMPT, DIAGNOSIS_PROMPT, APPEAL_PROMPT, SHORT_REPLY_PROMPT } from "./prompts";
import { cache } from "./store";
import { categoryComplianceData } from "../data/compliance-data";

// SECURITY: All requests MUST go through Cloudflare Worker in production.
// In dev mode (localhost:5173), we proxy through Vite dev server to avoid CORS.
// API keys are NEVER exposed in frontend bundle — only available in Cloudflare Worker.
// Vercel Serverless Functions are deprecated; all AI traffic routes through Workers.
...
async function callAI<T>(endpoint: string, params: Record<string, unknown>): Promise<T> {
  try {
    ...
    const CLOUDFLARE_WORKER_URL = "https://mini-disco-api.senliri028.workers.dev";
